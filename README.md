...
- **Real photos:** imported players render their actual headshot — a local cached copy in `assets/faces/p<id>.png` first, the live URL from the data as fallback, then your own files (`assets/faces/<name>.png`), then generated art. Club badges are 256px crests from [football-logos.cc](https://football-logos.cc) (map in `js/data/badges_remote.js`, regen via `node tools/build_remote_badges.mjs`), with generated SVG crests as fallback.
